<?php
    ...
    try{
        ...
        $query="SELECT p.product_id, p.product_name, i.src_thb, i.alt, p.newPrice,p.oldPrice, p.sale, p.category_id, p.brand_id, p.gender_id, p.inStock, p.availableSizes, p.material, p.coo FROM product p INNER JOIN pictures i ON p.product_id= i.product_id WHERE 1";
        if($pol!=""){
        ...
        foreach($result as $red){
            ...
            <div class='card text-center card-product'>
              <div class='card-product__img '>
                <a href='index.php?pages=single-product&id=".$red->product_id."'><img class='card-img slikaProizvoda' src='assets/".$red->src_thb."' alt='Slika ".$red->alt."' value=".$red->product_id." id='proizvod'></a>
              </div>
              <div class='card-body'>
                </br>
                <h4 class='card-product__title'><a href='index.php?pages=single-product&id=".$red->product_id."'>".$red->product_name."</h4></a>";
        ...
        if($n_o_pages>1){
            ...
            for($i=0;$i<$n_o_pages;$i++){
                $output.="<li class='page-item'><a class='page-link' href='index.php?pages=shop&page=".($i+1)."'>".($i+1)."</a></li>";
            }
            $output.="</ul></div>";
        }
