...
function fetchProductsIndex(){
    ...
    $query="SELECT p.product_id, p.product_name, i.src_thb ,i.alt, c.category_name, p.oldPrice, p.newPrice FROM product p INNER JOIN pictures i ON p.product_id = i.product_id INNER JOIN category c ON p.category_id = c.category_id WHERE p.sale=1";
    ...
    for($i=0;$i<count($result);$i++){
        ...
        <div class='card text-center card-product'>
          <div class='card-product__img'>
            <a href='index.php?pages=single-product&id=".$result[$i]->product_id."'>
            <img class='card-img' src='assets/".$result[$i]->src_thb."' alt='".$result[$i]->alt."'>
            </a>
          </div>
          <div class='card-body'>
            <p>".$result[$i]->category_name."</p>
            <h4 class='card-product__title'><a href='index.php?pages=single-product&id=".$result[$i]->product_id."'>".$result[$i]->product_name."</a></h4>";
...
function fetchAttempts($username){
    global $connection;

    $query="SELECT loginAttempts FROM user WHERE username=:username";
    $prepare=$connection->prepare($query);
    $prepare->bindParam(":username",$username);
    $prepare->execute();
    $result=$prepare->fetch();
...
function updateAttempts($username,$loginAttempts){
    global $connection;

    $query="UPDATE user SET loginAttempts=:loginAttempts WHERE username=:username";
    $prepare=$connection->prepare($query);
    $prepare->bindParam(":loginAttempts",$loginAttempts);
    $prepare->bindParam(":username",$username);
    $result=$prepare->execute();
...
function resetTime($username){
    global $connection;

    $query="UPDATE user SET attempt_date=:date WHERE username=:username";
    $prepare=$connection->prepare($query);
    $prepare->bindParam(":date",new DateTime());
    $prepare->bindParam(":username",$username);
    $result=$prepare->execute();
...
function displayLogFile(){
    ...
    if(trim($output)==""){
        $output="<tr><td colspan='2'>Nothing in log file.</td></tr>";
    }
...
function fetchWebsiteStats(){
    ...
    $pages=["Login","Register","Cart","Checkout","Contact","Shop","Single product","Verification","Add product","Add survey","Add user","Admin panel","Answer message","Update product","Update survey","Update user","View order stats","View poll stats","Website log file","Index"];
    $file=fopen('data/log.txt',"r");
    if($file){
        while(($line=fgets($file))!==false){
            $parts=explode("\t",$line);
            $br++;
            switch(trim($parts[1])){
                case "Login" : $count[0]++;break;
                case "Register" : $count[1]++;break;
                case "Cart" : $count[2]++;break;
                case "Checkout" : $count[3]++;break;
                case "Contact" : $count[4]++;break;
                case "Shop" : $count[5]++;break;
                case "Single product" : $count[6]++;break;
                case "Verification" : $count[7]++;break;
                case "Add product" : $count[8]++;break;
                case "Add survey" : $count[9]++;break;
                case "Add user" : $count[10]++;break;
                case "Admin panel" : $count[11]++;break;
                case "Answer message" : $count[12]++;break;
                case "Update product" : $count[13]++;break;
                case "Update survey" : $count[14]++;break;
                case "Update user" : $count[15]++;break;
                case "View order stats" : $count[16]++;break;
                case "View poll stats" : $count[17]++;break;
                case "Website log file" : $count[18]++;break;
                case "Index" : $count[19]++;break;
                default : $count[19]++;  
            }
        }
    }
...
function fetchUsersLogged(){
    ...
    if($br==0){
        $output="<tr><td colspan='2'>No users logged for today.</td></tr>";
    }else{
        $output.="<tr>
...
function attemptStart($username){
    global $connection;
    $time=getdate();
    $query="UPDATE user SET attempt_date=NOW() WHERE username=:username";
    $prepare=$connection->prepare($query);
    $prepare->bindParam(":username",$username);
    $result=$prepare->execute();
...
function attemptDate($username){
    global $connection;

    $query="SELECT attempt_date FROM user WHERE username=:username";
    $prepare=$connection->prepare($query);
    $prepare->bindParam(":username",$username);
    $prepare->execute();
    $result=$prepare->fetch();
...
function fetchSliderProducts(){
    ...
    $query="SELECT p.product_id, p.product_name, i.src_thb, i.alt, c.category_name, p.sale, p.inStock, p.newPrice FROM product p INNER JOIN pictures i ON p.product_id= i.product_id INNER JOIN category c ON p.category_id=c.category_id WHERE p.inStock=1 AND p.sale=1";
    ...
    for($i=0;$i<count($result);$i++){
        ...
        <div class='card-product__img'>
            <a href='index.php?pages=single-product&id=".$result[$i]->product_id."'>
                <img class='img-fluid' src='assets/".$result[$i]->src_thb."' alt='".$result[$i]->alt."'/>
            </a>
        </div>
        <div class='card-body'>
          <p>".$result[$i]->category_name."</p>
        ...
        foreach($result as $row){
            $output.="<tr>
                        ...
                        if($row->answered){
                            ...
                            $output.="<td><a class='btn btn-primary' href='index.php?pages=admin/answerMessage&id=".$row->message_id."'>Answer</a></td>";
                        }
                    $output.="</tr>";
        }
    }else{
...
function fetchMessage($id){
    ...
    $query="SELECT name,subject, message,email_user,date FROM admin_message WHERE message_id=:id";
    $prepare=$connection->prepare($query);
...
function fetchSurveyAdmin(){
    ...
    if($result){
        foreach($result as $row){
            ...
            $output.="</td>
            <td>".$row->poll_date."</td>
            <td><a href='index.php?pages=admin/view-stats&id=".$row->poll_id."'>View</a></td>
            </tr>";
        }
    }else{
    ...
    if($result){
        foreach($result as $row){
            $output.="<tr>
                ...
                <td>".$row->total."$</td>
                <td>".$row->date."</td>
                <td><a href='index.php?pages=admin/view-order&id=".$row->receipt_id."'>View</a></td>
            </tr>";
        }
    }else{
...
function fetchOrderDetails($id){
    ...
    foreach($result as $row){
        $output.="<tr>
            ...
            <td><a href='index.php?pages=single-product&id=".$row->product_id."'>".$row->product_name."</a></td>
            <td>".$row->price."</td>
            <td>".$row->size."</td>
            <td>".$row->quantity."</td>
        </tr>";
...
function updateSurvey($id,$name, $status){
    global $connection;

    $query="UPDATE poll SET poll_name=:name, status=:status WHERE poll_id=:id";
    $prepare=$connection->prepare($query);
    $prepare->bindParam(":name",$name);
    $prepare->bindParam(":status",$status);
    $prepare->bindParam(":id",$id);
