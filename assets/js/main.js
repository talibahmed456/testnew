...
function SkrolGore() {
    ...
    if(pages=="admin/addUser" || pages=="admin/updateUser" || pages=="adminpnl" || pages=="admin/addSurvey" || pages=="admin/updateSurvey" || pages=="admin/addProduct" || pages=='admin/updateProduct'){
        $("select").niceSelect();
    }
    if(pages=="shop"){
        filterProducts();
...
function proveriIme() {
  ...
  function proveraCheckBox(){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                if(result.answer.substring(0,3)=="<h6"){
                    ...
                    const timer=setTimeout(function(){
                        window.location.href='index.php?pages=adminpnl';
                    },1000);
                }else if(result.answer.substring(0,3)=="<h4"){
                    const timer=setTimeout(function(){
                        window.location.href='index.php?pages=verification';
                    },1000);
                }
                if(url=='models/sendMessage.php'){
                    const timer=setTimeout(function(){
                        window.location.href='index.php?pages=contact'
                    },2000);
                }
            },
            error: function(xhr){
    ...
    $("#btnAnswerMessage").click(function(){
        let message=$("#messageAnswer").val();
        const urlParams = new URLSearchParams(window.location.search);
        let id=urlParams.get('id');
        if(message.trim()!=""){
            $("#upozorenjeText").addClass('hidden');
            let paket={
                id:id,
                answer:message
            }
            slanjePodataka('models/adminpnl/answerMessage.php',paket,'statusAnswer');
        }else{
            $("#upozorenjeText").removeClass('hidden');
            $("#upozorenjeText").css('color','red');
        }
    });
